...
### In�cio das defini��es de m�todos
...
### In�cio da leitura da fita_espelho
...
fita_espelho.each_line do |linha|
  ...
  case linha[17...18]
    ...
    when '2'
      ...
      #Verifica se o @servidor est� na base, ser� utilizado em v�rios m�todos
      ...
      ###Inicia a inser��o dos dados no bando de dados

      puts "\n"

    puts ":upagadora => #{@servidor[:upagadora]}      :matricula => #{@servidor[:matricula] } \n"
    puts ":matricula_dv => #{@servidor[:matricula_dv]}:tipoRegistro => #{@servidor[:tipoRegistro] } \n"
    puts ":nome => #{ @servidor[:nome]}               :cpf => #{ @servidor[:cpf] } \n \n"

    puts ":funcao => #{@servidor[:funcao]} :unidadeOrganizacionalFuncao => #{@servidor[:unidadeOrganizacionalFuncao]} \n"
    puts ":unidadeOrganizacionalLotacao => #{@servidor[:unidadeOrganizacionalLotacao]}\n"
    puts ":siglaNovaFuncao => #{@servidor[:siglaNovaFuncao]} :unidadeOrganizacionalNovaFuncao => #{@servidor[:unidadeOrganizacionalNovaFuncao]} \n"
...
###################################################################################################################################
      ...
      dbh.query("UPDATE crp_pessoa SET CD_VINCULO=#{@servidor[:vinculo]},
                                         ...
                                         CD_PIS_PASEP='#{@servidor[:pis_pasep]}'
        ...
        dbh.query("INSERT INTO crp_pessoa (CD_VINCULO,
                                           ...
                                           CD_NACIONALIDADE,
                                           CD_RG,
                                           CD_PIS_PASEP) values (#{@servidor[:vinculo]},
                                                                '#{@servidor[:nome]}',
...
###################################################################################################################################
#######################    Insere ou Atualiza Pessoa F�sica    ####################################################################
###################################################################################################################################
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Pessoa F�sica    #############################################################
###################################################################################################################################
...
###################################################################################################################################
      ...
      #Insere Unidade Organizacional nova se ela n�o existir no banco
      ...
      #Insere Unidade Organizacional v�lida se ela n�o existir no banco
...
###################################################################################################################################
      ...
      if cd_situacao_base.nil?
        #puts "    Inserindo situa��o do @servidor..."
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Situa��o    ##################################################################
###################################################################################################################################

###################################################################################################################################
#######################    Insere ou Atualiza Fun��es    ##########################################################################
###################################################################################################################################
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Fun��es    ###################################################################
###################################################################################################################################
...
###################################################################################################################################
      ...
      #VERIFICAR A DATA DA FITA-ESPELHO POIS ESTE M�TODO PODE ATUALIZAR UMA SITUACA��O ANTIGA
      if @servidor[:ja_existente] == false
...
###################################################################################################################################
#######################    Insere ou Atualiza Endere�o Servidor   #################################################################
###################################################################################################################################

      #M�todo feito em fun��o dos relacionamento pessoa x endereco ser: um para um
      ### Sem tratamento de erros ###
      case @servidor[:pais]
        when "073":
            pais = "HT"
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Endere�o Servidor    #########################################################
###################################################################################################################################
...
###################################################################################################################################
      ...
      if @servidor[:grupo] != "000"
        ...
        #SE O @servidor N�O TEM CARGO NA BASE E TEM CARGO NA FITA ENT�O INSERIR:
        ...
        #SE O C�DIGO DO GRUPO+CARGO FOR DIFERENTE NA FITA E NO BANCO ENT�O *ATUALIZE O ULTIMO DO BANCO* E INSIRA O NOVO
...
###################################################################################################################################
#######################    Insere ou Atualiza Fun��o Servidor    ##################################################################
###################################################################################################################################

      #se o @servidor possuir fun��o no banco sem data de sa�da e na fita n�o existir
      #relato sobre fun��es, ent�o o m�todo vai verificar exclus�o do @servidor e atua-
      #lizar a sa�da da fun��o com a entrada da ocorr�ncia de exclus�o

      #Verifica a existencia das fun��es
      ...
      #Caso tenha fun��o na fita, formata as datas e pega os valores da funcao da fita
      if tem_funcao_fita
        data_saida_funcao_fita = formata_data(@servidor[:dataSaidaFuncao])
        #Verifica se tem data de sa�da na fita
        ...
        #N�O EXISTE O M�TODO FIRST PARA NilClass
      ...
      #Caso n�o tenha fun��o na fita, verifica registro da data de sa�da da fun��o(data de exclus�o)
      else
        #Verifica se possui ocorrencia de exclus�o na fita
        ...
        #Atualiza com data de exclus�o a �ltima fun��o
      ...
      #Caso haja registro de nova fun��o na fita, extrai-se os dados de ingresso e sa�da e a unidade organizacional referente � nova fun��o
...
#          data_saida_nova_funcao_fita = nil
        ...
        #Se existe chave de nova fun��o, n�o precisa inserir nova fun��o no banco. A rec�proca � verdadeira.
        cd_nova_funcao_servidor ? precisa_inserir_nova_funcao_no_banco = false : precisa_inserir_nova_funcao_no_banco = true
      end

      #verifica se a data de saida � menor que...
      ...
      ##PRESTAR ATEN��O PARA O VERIFICAR DA DATA DE INGRESSO, POIS N�O PRECISA SER EXATAMENTE A DATA DE INGRESSO, PODE SER UM CAMPO NULO
      ...
      if precisa_inserir_funcao_no_banco == true
        ...
        if esta_em_uma_funcao == true
          ...
          if funcao_data_anterior_2011 == true
            #ATUALIZA��O PARA CASOS ANTERIORES A 2011-01-01 E COM DATA DE SAIDA PREENCHIDA NA FUN��O
            #A DATA DE SAIDA PRECISA ESTAR PREENCHIDA SEN�O CAIRA NO ELSE DO PROXIMO IF, QUE
            #ATUALIZA A SAIDA COM DATA 2011-01-01, POIS A FITA VEM COM O REGISTRO DA FUN��O APENAS COM
            # A UORG ALTERADA, COMO SE O @servidor SEMPRE ESTIVESSE ESTADO NAQUELA FUN��O
            ...
            #ATUALIZA��O PARA CASOS EM QUE A ENTRADA ATUAL NO BANCO � MAIOR OU IGUAL A 2011-01-01
            #SIGNIFICA QUE J� EST� REGISTRADA A MUDAN�A DE ESTRUTURA DE UORG
            if data_entrada_funcao_atual >= "2011-01-01"
              #A MAIORIA DOS CASOS EST� NESSA CONDI��O
              #puts "CASO COMUM"
              ...
              #SEN�O CAI NO CASO DE TRANSI��O DE ESTRUTURA DE UNIDADE ORGANIZACIONAL 2010/12 --> 2011/01
              #A SAIDA � ATUALIZADA COM DATA DE '2011-01-01'
        ...
        #ESTE SEN�O CONCLUI QUE N�O H� RAZ�ES L�GICAS PARA INSER��O DE UM REGISTRO DE HISTORICO
      ...
      else #Se n�o precisa inserir nova fun��o no banco
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Fun��o Servidor   ############################################################
###################################################################################################################################
...
###################################################################################################################################
      ...
      #verifica que fun��o vem na fita
      @servidor[:codigo_nivel] != "00000"? tem_funcao_fita = true : tem_funcao_fita = false
      @servidor[:codigoNivelNovaFuncao] != "00000"? tem_nova_funcao_fita = true : tem_nova_funcao_fita = false
      ...
      #so usa a lotacao se n�o existirem fun��es ou estarem com data de saida preenchida
      if tem_nova_funcao_fita
      ...
      else #Caso n�o tenha nova fun��o fita
        if tem_funcao_fita
      ...
      #se n�o tiver que usar lota��o(pois h�, fun��o ou nova fun��o)
      if !usar_lotacao
      ...
      #verifica uorg da nova funcao no historico do banco
        ...
        else #caso n�o tenha nova funcao fita
      ...
      #se n�o tiver fun��o e nova fun��o utiliza a lota��o(que sempre vai existir)
      ...
      #   puts "tem_campo_na_base ---------------------> #{tem_campo_na_base }"
          if tem_campo_na_base == true
            #\/\/ esse if � utilizado para campos na base que possuem uorg e data zerados
      ...
      #       essa atualiza��o surgiu da mudan�a de codigo de uorg ent�o ele vai inserir data de 2011-01-01
      ...
      #    puts "precisa_inserir_uorg_funcao_fita == true and precisa_inserir_uorg_nova_funcao_fita == true"
          #verifica se existe aquela unidade j� inserida com data de saida nula
      ...
      #puts "FUNCAO: INSERINDO DADOS COMPLETOS DE UORG FUNCAO POIS TEM TAMB�M NOVA FUNCAO"
            dbh.query("INSERT INTO crp_unidade_organizacional_hist (CD_PESSOA_FISICA,
                                                                     CD_UNIDADE_ORGANIZ,
                                                                     DT_INGRESSO_LOTACAO,
                                                                     DT_SAIDA_LOTACAO)
      ...
      #se tem campo na base  e nao tem entrada e a saida da fun��o na fita � nula: significa que a pessoa s� mudou de uorg em 2011-01-01
      #puts "/n /n /n /n /n"
      #puts "tem_campo_na_base - #{tem_campo_na_base}"
      ...
      #atualiza a data de saida do ultimo historico com a data de entrada do @servidor na fun��o caso a sa�da seja nula
      ...
      #se precisa inserir, pois nao achou uorg igual nem entrada igual:
        if precisa_inserir_uorg_nova_funcao_fita == true
          ...
          if !existe_uorg_funcao_saida_nula3
            if tem_campo_na_base == true
      #atualiza a data de saida do ultimo historico com a data de entrada do @servidor na nova fun��o caso a sa�da seja nula
...
###################################################################################################################################
#######################    Insere ou Atualiza Ocorr�ncia Ingresso Servi�o P�blico   ###############################################
###################################################################################################################################
      ...
      #      puts "    Inserindo ocorrencias de ingresso nos servi�o publico..."
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Ocorr�ncia Ingresso Servi�o P�blico   ########################################
###################################################################################################################################

###################################################################################################################################
#######################    Insere ou Atualiza Ocorr�ncia Ingresso �rg�o   #########################################################
###################################################################################################################################
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Ocorr�ncia Ingresso �rg�o   ##################################################
###################################################################################################################################

###################################################################################################################################
#######################    Insere ou Atualiza Ocorr�ncia Exclus�o   ###############################################################
###################################################################################################################################
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Ocorr�ncia Exclus�o   ########################################################
###################################################################################################################################

###################################################################################################################################
#######################    Insere ou Atualiza Ocorr�ncia Afastamento   ############################################################
###################################################################################################################################
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Ocorr�ncia Afastamento   #####################################################
###################################################################################################################################

###################################################################################################################################
#######################    Insere ou Atualiza Ocorr�ncia Inatividade   ############################################################
###################################################################################################################################
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Ocorr�ncia Inatividade   #####################################################
###################################################################################################################################

###################################################################################################################################
#######################    Insere ou Atualiza Ocorr�ncia Modifica��o Funcional   ##################################################
###################################################################################################################################
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Ocorr�ncia Modifica��o Funcional   ###########################################
###################################################################################################################################

###################################################################################################################################
#######################    Insere ou Atualiza Ocorr�ncia Aposentadoria   ##########################################################
###################################################################################################################################
...
###################################################################################################################################
#######################    Fim de Insere ou Atualiza Ocorr�ncia Aposentadoria   ###################################################
###################################################################################################################################

  end #--FIM DO CASE E EXTRA��O DE DADOS
end #-- Fim da leitura do arquivo
  dbh.query("UPDATE crp_funcao_servidor SET DT_SAIDA_FUNCAO = null
             where dt_saida_funcao = '0000-00-00'")
